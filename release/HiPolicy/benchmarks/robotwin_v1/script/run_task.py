from execution.runtime import collection_config, seed_file
import sys
sys.path.append('./')
...
def main():
    task_name = os.environ.get("HIPOLICY_TASK") or input()
...
def run(Demo_class, args):
    ...
    if not args['use_seed']:
        ...
        with open(seed_file(args['task_name']), 'w') as file:
        ...
        with open(seed_file(args['task_name']), 'r') as file:
    ...
    if args['collect_data']:
        ...
        for id in range(args['st_episode'], args['episode_num']):
            ...
            # modify start
            info_file_path = args['save_path'] + '_pkl/scene_info.json'
            os.makedirs(args['save_path'] + '_pkl', exist_ok=True)
            ...
            if Demo_class.save_type.get('raw_data', True):
                head_config = Demo_class.get_camera_config(args['head_camera_type'])
                left_config = Demo_class.get_camera_config(args['wrist_camera_type'])
                right_config = Demo_class.get_camera_config(args['wrist_camera_type'])
