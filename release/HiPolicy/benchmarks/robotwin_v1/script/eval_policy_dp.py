from execution.adapter import PolicyAdapter
from execution.runtime import usr_variant
import sys
sys.path.append('./') 
...
def test_policy(task_name, Demo_class, args, dp, st_seed, test_num=20):
    expert_check = True
    ...
    while succ_seed < test_num:
        ...
        Demo_class.setup_demo(now_ep_num=now_id, seed = now_seed, is_test = True, ** args)
        getattr(Demo_class, "apply_" + usr_variant())(dp, args)
...
def main(usr_args):
    ...
    suc_nums = []
    test_num = usr_args.eval_episodes 
    topk = 1

    dp = PolicyAdapter(usr_args.checkpoint, usr_args.variant, usr_args.entropy_guidance)

    if usr_args.variant == "dp3":
        import eval_policy_dp3
        eval_policy_dp3.TASK = task_name
        st_seed, suc_num = eval_policy_dp3.test_policy(task, args, dp, st_seed, test_num=test_num)
    else:
        st_seed, suc_num = test_policy(task_name, task, args, dp, st_seed, test_num=test_num)
    suc_nums.append(suc_num)

    topk_success_rate = sorted(suc_nums, reverse=True)[:topk]
    save_dir = Path(usr_args.result_dir)
