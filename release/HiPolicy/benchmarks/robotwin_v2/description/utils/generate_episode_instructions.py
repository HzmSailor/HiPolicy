...
def load_scene_info(task_name: str, setting: str, scene_info_path: str) -> Dict[str, Dict]:
    ...
    file_path = os.path.join(parent_directory, "../..", scene_info_path, task_name, setting, "scene_info.json")
    try:
        with open(file_path, "r") as f:
...
def save_episode_descriptions(task_name: str, setting: str, generated_descriptions: List[Dict]):
    ...
    output_dir = os.path.join(os.environ.get("HIPOLICY_RAW_ROOT", os.path.join(parent_directory, "../../data")), task_name, setting, "instructions")
...
if __name__ == "__main__":
    ...
    scene_info = load_scene_info(args.task_name, args.setting, os.environ.get('HIPOLICY_RAW_ROOT', args_dict['save_path']))
