...
class ConditionalUnet1D(nn.Module):
    def __init__(self, input_dim, actual_horizon=8, local_cond_dim=None, global_cond_dim=None, diffusion_step_embed_dim=256, down_dims=[256, 512, 1024], kernel_size=3, n_groups=8, cond_predict_scale=False):
    ...
    def forward(self, sample: torch.Tensor, timestep: Union[torch.Tensor, float, int], local_cond=None, global_cond_long=None, global_cond_mid=None, global_cond_short=None, **kwargs):
        if benchmark() == "robotwin_v1":
            """
            x: (B,T,input_dim)
            ...
            for idx, (resnet, resnet2, upsample) in enumerate(self.up_modules):
                ...
                if idx == len(self.up_modules) and len(h_local) > 0:
                    x = x + h_local[1]
                x = resnet2(x, global_feature_long, global_feature_mid, global_feature_short)
                # print("resnet2之后：x.shape: ", x.shape)
                x = upsample(x)
                # print("upsample之后：x.shape: ", x.shape)
