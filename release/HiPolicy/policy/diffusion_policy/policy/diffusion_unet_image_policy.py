# Benchmark branches preserve original operations and checkpoint layouts.
...
class DiffusionUnetImagePolicy(BaseImagePolicy):
    ...
    # ========= inference  ============
    def conditional_sample(self, condition_data, condition_mask, local_cond=None, global_cond_long=None, global_cond_mid=None, global_cond_short=None, generator=None, num_samples=1, **kwargs):
        ...
        if benchmark() == "robotwin_v1":
            ...
            # 如果 num_samples > 1，扩展 batch 维度以并行采样
            ...
            for t in scheduler.timesteps:
                ...
                # 2. predict model output
                model_output = model(trajectory, t, local_cond=local_cond, global_cond_long=global_cond_long, global_cond_mid=global_cond_mid, global_cond_short=global_cond_short)
    ...
    def predict_action(self, obs_dict: Dict[str, torch.Tensor], num_samples: int=1):
        if benchmark() == "robotwin_v1":
            """
            ...
            action_chunks = torch.cat([action_pred_long, action_pred_mid, action_pred_short], dim=1)
            if num_samples > 1:
                action_chunks = unpack_candidates(action_chunks, B, num_samples)
                action = unpack_candidates(action, B, num_samples)
                action_pred = action
            result = {
                'action': action,
            ...
            """
            obs_dict: must include "obs" key
            num_samples: 并行采样的样本数量
            result: must include "action" key
                    如果 num_samples > 1，返回的 action shape 为 (num_samples, B, T, Da)
                    如果 num_samples == 1，返回的 action shape 为 (B, T, Da)
            """
            assert "past_action" not in obs_dict  # not implemented yet
            # normalize input
            nobs = self.normalizer.normalize(obs_dict)
            ...
            if self.obs_as_global_cond:
                ...
                obs_long_indices = [0, 3, 6]
                obs_mid_indices = [2, 4, 6]
                obs_short_indices = [4, 5, 6]
                this_nobs_long = dict_apply(nobs, lambda x: x[:, obs_long_indices, ...].reshape(-1, *x.shape[2:]))
                this_nobs_mid = dict_apply(nobs, lambda x: x[:, obs_mid_indices, ...].reshape(-1, *x.shape[2:]))
                this_nobs_short = dict_apply(nobs, lambda x: x[:, obs_short_indices, ...].reshape(-1, *x.shape[2:]))
                nobs_features_long = self.obs_encoder(this_nobs_long)
                nobs_features_mid = self.obs_encoder(this_nobs_mid)
                nobs_features_short = self.obs_encoder(this_nobs_short)
                # reshape back to B, Do
            ...
            # 如果 num_samples > 1，需要 reshape 回 (num_samples, B, T, Da)
            if num_samples > 1:
            ...
            result = {"action": action, "action_pred": action_pred, "action_chunks": action_pred}
            return result
    ...
    def compute_loss(self, batch):
        if benchmark() == "robotwin_v1":
            ...
            action_long_indices = [7, 10, 13, 16, 19, 22, 25, 28]
            action_mid_indices = [7, 9, 11, 13, 15, 17, 19, 21]
            action_short_indices = [7, 8, 9, 10, 11, 12, 13, 14]

            nactions_long = nactions[:, action_long_indices, ...]
            nactions_mid = nactions[:, action_mid_indices, ...]
            nactions_short = nactions[:, action_short_indices, ...]

            trajectory = torch.cat([nactions_long, nactions_mid, nactions_short], dim=1)
            ...
            # Predict the noise residual
            pred = self.model(noisy_trajectory, timesteps, local_cond=local_cond, global_cond_long=global_cond_long, global_cond_mid=global_cond_mid, global_cond_short=global_cond_short)
